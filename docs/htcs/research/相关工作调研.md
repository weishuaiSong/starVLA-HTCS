...
> **2026-05-14 增补**：§1.2 加入 **RoboMemArena & PrediMem**（2026.05，arXiv:2605.10921）——首个把"记忆依赖性"列为显式设计指标的机器人记忆基准（26 任务 / 平均 >1000 步 / 68.9% 记忆依赖子任务）+ 配套 PrediMem 方法（双系统 VLA + recent/keyframe 双缓冲 + 预测编码头）。
...
| 长时尺度承载 | **VLA 内 chain-of-thought 滚动语言摘要串**（π_HL 自回归生成 m_t + l_t；训练用教师 LLM 摘要监督，推理无外部 LLM） | 多模态 prompt（subgoal img + metadata） | 外置可训练记忆库 | 块级循环 query |
...
2. **补一条长时尺度**：可借鉴 MEM 的符号化语言摘要路径（VLA 内 chain-of-thought 生成，可解释、可审计；训练需教师摘要监督），或 MemoryVLA / ReMem-VLA 的隐向量路径（纯端到端、不需要文本监督）。
3. **关键决策点**：长时记忆**是否符号化**——若要支持失败重试与人类干预，语言串路径更友好；若以纯部署效率为先，循环 query 路径更轻量（且省去摘要监督数据准备）。
...
| **RoboMemArena & PrediMem: A Comprehensive and Challenging Robotic Memory Benchmark**<br>*Lei et al., 2026.05 (arXiv:2605.10921)* | **基准 + 方法**双重贡献。基准：26 个任务、平均轨迹 >1000 步、**68.9% 子任务具有记忆依赖**，VLM 辅助自动设计/组合子任务并提供多模态记忆注释，支持真实物理评估。配套方法 **PrediMem**：**双系统 VLA**——高层 VLM 规划器管理含 **recent + keyframe 双缓冲**的记忆库，并接一个**predictive coding head** 提升对任务动态的敏感性。报告"PrediMem 优于所有 baseline"（未披露具体数字）。 | 现有 VLA 基准缺乏系统性的长时记忆挑战，主流记忆方法也没有专门面向"记忆依赖子任务"的公平评测口径；同时单一记忆容器难以兼顾"近期细节"与"关键事件"。 | **(1) 基准侧**：用 VLM 自动设计子任务并按记忆依赖类型组合长轨迹；提供子任务指令 + 关键帧注释 + 真机评估接口——填补 LIBERO/CALVIN 等不考察长程记忆的口径。<br>**(2) PrediMem 双系统**：高层 VLM 规划器（与 MemER 同构）+ 低层动作策略，**两类缓冲分担两个时间尺度**——recent buffer 承载短时观测、keyframe buffer 承载选择性长期事件。<br>**(3) Predictive coding head**：让模型预测未来任务动态而非仅匹配当前观察，强化记忆对"变化"的敏感度——**理念上与 ReMem-VLA 的 POP（Past Observation Prediction）镜像**：POP 预测过去，PrediMem 预测未来。 |
...
- **RoboMemArena 是这一支首次正式提出"记忆依赖性"为基准设计指标**：68.9% 子任务带显式记忆依赖、平均轨迹 >1000 步——以往 LIBERO-Long / CALVIN ABC-D 是"任务长"但**不强制要求历史**。新工作若主打"任务条件化历史压缩"，**应在 RoboMemArena 上跑一遍以拿到针对性证据**，单靠 LIBERO 难以区分"记得住"与"不需要记"。
- **PrediMem 的 predictive coding head 与 ReMem-VLA 的 POP 形成镜像对**：POP 让模型预测**过去**观察以强迫记忆保留可追溯信号；PrediMem 让模型预测**未来**动态以提升对变化的敏感度——两者都在用"辅助预测任务"约束记忆质量，方向相反。这对新工作设计辅助 loss 是个直接可参考的二选一。
...
|---|---|---|---|---|---|---|---|
| **MEM** | 多尺度 | 视频编码 + VLA 内 chain-of-thought 滚动语言摘要 | 秒级 (ViT 内时空注意力) | 分钟级 (π_HL 自回归生成的摘要字符串，训练用教师 LLM 监督) | ✗ | 中（ViT 内插时间注意力 + π_HL/π_LL 分头解码） | 2026.03 |
...
| **RoboMemArena / PrediMem** | 外置/检索 + 预测编码 | Recent + Keyframe 双缓冲 + predictive coding head | recent buffer | keyframe buffer | ✓（高层 VLM 检索） | 大（双系统 + 预测头） | 2026.05 |
...
- ⭐ **[RoboMemArena & PrediMem — arXiv:2605.10921](https://arxiv.org/abs/2605.10921)** — 2026.05，机器人记忆专用基准（26 任务 / 1000+ 步 / 68.9% 记忆依赖）+ 双系统 VLA（recent+keyframe 双缓冲 + 预测编码头）
